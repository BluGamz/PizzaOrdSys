...
struct PizzaTypes{int smallQuantity,mediumQuantity,largeQuantity,others,freshPan,cheeseBurst,wheatThinCrust;};
struct PizzaCost{int smallCost,mediumCost,largeCost,freshPanCost,cheeseBurstCost,wheatThinCrustCost;};

main() //Uses numbers to convey the different types of pizza available in the system
{
	printf("PIZZA ORDERING SYSTEM\n\n");
 	...
 	char name[20];
 	char itemNames[12][100]= //List of current pizzas in system that the customer can choose from
	{
  		"Margherita(Veg)",
 	...
 	for(i=0;i<12;i++) //Creates quantity, freshness, thin creust, extra cheese and other variables
	{
	...
	i=0;
 	while(i<24) //Creates cost variables for the contents of the pizza
	{
	...
	do //Creates the flow of order taking that is revealed to the customer
	{
		if(c==1) //Pizza Categories
		{
		   	printf("Enter\n1 - Veg Pizzas\n2 - Chicken Pizzas\n3 - Sides\n4 - Exit\n\n");
		    scanf("%d",&mainChoice);
		   	switch(mainChoice)
		...
		if(c==2) //Changeing Pizza Quantity
		{
		...
		if(c==4) //Delete Pizza from Order
		{
		   	int id;
		...
		if(c==3) //Uses other method to display current order
		{
		   	DisplayOrder(pizzaCart,costOnSize,itemNames,sidesCost);
		}
...
void DisplayOrder(struct PizzaTypes pizzaCart[100],struct PizzaCost costOnSize[12],char itemNames[12][100],int sidesCost[4]) //Creates the general display for the order
{
 	int totalCost=0,i;
 	printf("---------------------List in Cart-------------------------\n");
 	...
 	for(i=0;i<11;i++)
 	{
  		if(pizzaCart[i].smallQuantity>0||pizzaCart[i].mediumQuantity>0||pizzaCart[i].largeQuantity||pizzaCart[i].others>0||pizzaCart[i].freshPan>0||pizzaCart[i].cheeseBurst||pizzaCart[i].wheatThinCrust>0)
 		{
   			if(i>=0&&i<=7)
			{
				...
				totalCost+=pizzaCart[i].smallQuantity*costOnSize[i].smallCost;			 //Calculates the cost of the pizza based on the customers quantity, size, freshness, extra cheese and special crust
			...
			{
   				printf("Id : %d\n",i);									  //Displays the Id of the customer, item name, item quantity, and cost of order
   				printf("Item Name : %s\n",itemNames[i]);
   				printf("Quantity : %d\n",pizzaCart[i].others);
